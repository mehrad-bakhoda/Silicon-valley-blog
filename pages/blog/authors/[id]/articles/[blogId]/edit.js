...
function EditABlog({ access, article }) {
  //Variables

  // useStates

  const [arr, setArr] = useState(article.metaTags);
  ...
  const [header, setHeader] = useState(article.title);
  const [ttr, setTTR] = useState(article.ttr);
  ...
  const handlePost = (e) => {
    ...
    fetch(`/api/blog/author/post?edit=true&id=${article.id}`, {
      method: "POST",
...
export default checkAuthClient(EditABlog);
    ...
    if (context.req.headers.cookie) {
      ...
      if (payload.userId == checkIfExist.id) {
        const article = await prisma.article.findUnique({
          where: {
            id: parseInt(context.query.blogId),
          },
        });
        if (article.userId == payload.userId) {
